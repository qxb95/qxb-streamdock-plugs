...
def test_scan_registers_bundled_actions():
    assert set(ActionFactory._action_types) >= {'music'}
...
def test_create_action_extracts_name_from_dotted_action(plugin):
    ...
    action = ActionFactory.create_action('com.qxb.music.dummy', 'ctx', {'a': 1}, plugin)
...
def test_create_action_returns_none_for_unknown_type(plugin):
    assert ActionFactory.create_action('com.qxb.music.nope', 'ctx', {}, plugin) is None
...
def test_scan_is_noop_when_actions_dir_missing(monkeypatch):
    monkeypatch.setattr('core.action_factory.os.path.exists', lambda path: False)
